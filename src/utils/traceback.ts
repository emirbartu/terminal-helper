...
/**
 * Frame information extracted from traceback
 */
...
/**
 * Pattern matcher for tracebacks
 */
 ...
 */
export function isUserFile(p: string): boolean {
 ...
 */
export function extractFilesFromTraceback(log: string): Map<string, number> {
  ...
  if (userFrames.length > 0) {
    // Group by file path
    const fileGroups: Record<string, FrameInfo[]> = {};
 ...
 * @returns {FileContext} - Raw code snippet from the file.
 */
export function readFileContext(file: string, line: number, ctx = 30): FileContext {
 ...
 */
export function buildErrorContext(log: string, contextSize = 30, includeHeaders = true): string {
 ...
 * @param {number} [ctx=30] - Lines of context before and after (default is 30).
 */
export async function showSnippet(file: string, line: number, ctx = 30): Promise<void> {
  const start = Math.max(1, line - ctx), end = line + ctx;
  const cmd   = `sed -n '${start},${end}p' ${basename(file)}`;
  
  // Dynamic import to avoid circular dependencies
  const { echoCommand } = await import('../ui/terminalUI.js');
 ...
 */
export async function displaySnippetsFromError(log: string): Promise<void> {
 ...
 */
export function extractErrorLine(file: string, line: number): string {
 ...
 */
export function getErrorLines(log: string): string {
  const files = extractFilesFromTraceback(log);
  if (!files.size) return '';
