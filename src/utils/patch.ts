...
/**
 * Fix result from fixMissingCommentPlusSign
 */
 ...
 */
export async function showPatch(diff: string): Promise<void> {
  // Dynamic import to avoid circular dependencies
  const { BOX } = await import('../ui/terminalUI.js');
 ...
 */
function applyPatch(diff: string, cwd = process.cwd()): void {
 ...
 * @returns {FixResult} - Object containing fixed diff and whether it was fixed
 */
function fixMissingCommentPlusSign(diff: string): FixResult {
  const lines = diff.split('\n');
  let inHunk = false;
 ...
 */
export function extractDiff(diffText: string): string {
 ...
 */
export async function confirmAndApply(diff: string, cwd = process.cwd()): Promise<boolean> {
  // Display the patch
  await showPatch(diff);
  
  // Add spacing before confirmation prompt to prevent UI collision
  console.log('');
  
  // Ask for confirmation
  const { askYesNo } = await import('../ui/terminalUI.js');
 ...
 * @param {PatchData} patchData - The structured patch data
 * @param {string} _currentDir - The current working directory (unused but kept for API compatibility)
 * @returns {string} - Unified diff format text
 */
export function convertToUnifiedDiff(patchData: PatchData, _currentDir: string): string {
  // Group changes by file
  const fileChanges: Record<string, PatchChange[]> = {};
 ...
 * @param {PatchChange[]} changes - List of changes for a single file
 * @param {string} filePath - The path of the file being changed
 * @returns {string} - Formatted hunks
 */
function generateHunks(changes: PatchChange[], filePath: string): string {
  ...
  try {
    ...
    changes.forEach(change => {
      ...
      if (change.new_line !== null && change.new_line !== undefined) {
    ...
    console.log(chalk.red('Error generating hunks:', (error as Error).message));
 ...
 */
function processCodeLine(codeText: string, prefix: string, filePath: string): string {
 ...
 */
function detectStringLiteral(text: string, fileExt: string): boolean {
  ...
  switch (fileExt) {
    case 'py':
      ...
      return (
        ...
        (text.includes('"') && (text.match(/"/g)?.length || 0) >= 2) ||
        (text.includes("'") && (text.match(/'/g)?.length || 0) >= 2)
      );
